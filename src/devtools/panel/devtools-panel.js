...
document.getElementById("button_jquery").addEventListener("click", async () => {
  try {
    ...
    // const harLogPromise = browser.devtools.network.getHAR();
    // console.log("harLogPromise", harLogPromise);
    // const harLog = await harLogPromise;
    console.log("harLog", harLog);
  } catch (err) {
    console.error("error encountered attempting to get HAR");
    console.error(err);
  }

  browser.devtools.inspectedWindow.eval(checkjQuery).then(handlejQueryResult);
});
/**
